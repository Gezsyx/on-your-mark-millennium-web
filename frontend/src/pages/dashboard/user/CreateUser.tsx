    ...
    <div className="p-4">
      <div className="max-w-5xl mx-auto bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="border-b border-gray-100 px-8 py-6">
          ...
          <p className="text-gray-500 mt-1">Tambahkan data pengguna baru</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
          <div className="space-y-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className={`border rounded-xl px-4 py-3 focus:outline-none focus:ring-2 ${
                  errors.image ? "border-red-500 focus:ring-red-500" : "border-gray-200 focus:ring-blue-500"
                }`}
              />
          ...
          <div className="flex flex-col">
            <label className="text-sm font-semibold text-gray-700 mb-2">Image Preview</label>
            <div className="w-full aspect-square border border-dashed border-gray-300 rounded-2xl overflow-hidden bg-gray-50 flex items-center justify-center">
              {imagePreview ? (
                <img
                  ...
                  onError={(e) => { (e.target as HTMLImageElement).src = "https://placehold.co/400?text=Image+Tidak+Ditemukan"; }}
                />
              ) : (
                <div className="flex flex-col items-center text-gray-400">
                  <ImageIcon size={50} />
                  <p className="mt-3 text-sm">Preview image akan muncul di sini</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" onClick={() => navigate("/user")} className="px-5 py-2.5 rounded-xl border border-gray-300 hover:bg-gray-50 transition-colors">Cancel</button>
            <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 transition-all disabled:opacity-50">
              {loading ? <><Loader2 size={18} className="animate-spin" /> Loading...</> : "Create User"}
            </button>
          </div>
        </form>
      </div>
